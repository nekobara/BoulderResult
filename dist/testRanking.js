...
function testRanking() {
    ...
    let objs = testObj['results'];
    for (var i = 0; i < objs.length; i++) {
        let o = objs[i];
        let p = o['player'];
        console.log(o['num_of_top_complete']
    ...
    // 成績のロード
    let ranking = new BoulderResult_1.BlRanking(testObj.results);
    // 順位計算
    ranking.calcRanking();
    // 順位の表示
    console.log("\n##### 順位 #####");
    console.log("順位 順位P\tゼッケン 氏名");
    let results = ranking.getResults();
    ...
    */
    // 確認
    // 1位 暮瑠田 輪
    let r = ranking.getResultByPlayerId("12");
    if (r == null)
        console.log(r.getPlayer().getFullName() + "の成績データが正しくありません");
    if (r.getRanking() != 1)
        console.log(r.getPlayer().getFullName() + "の順位計算が間違っています。");
    // 2位 蔵井 民具
